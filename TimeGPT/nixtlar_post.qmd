...
Plots indicate that all of the series show periods of considerable volatility. The BE, DE, and FR series appear to be stationary. NP trends upward to the righe and the PJM series appears to be nonlinear.
```{r}
...
The `nixtla_client_forecast()` function is the main `nixtlar` forecastind function. (I have already run this function and saved the results RDS file in order not to make an API call every time the code is run during the blog building process.) 
...
The "no thought" prophet model does a pretty good job, but it does seem to have overacted to the downward trends at the beginnning and end of the forecast period. My guess is that with a little thought prophet could do better. 

The ARIMA model is a bit of a disappointment. I would have expected it to follow some of the twists and turns. It is as if the exponential smoothing and ARIMA models flipped a coin at the beginning of the forecast to decide who takes the high road and who gets the low road, but then tracked each other all the way through. However, to my way of thinking the exponential smoothing forecast is outstanding. It tracks the actual data closely for six forecast points. Thereafter, it looks like it is doing exactly what it is supposed to do. 


### Conclusion

It is clear that the TimeGPT model has upped the game for black box time series forecasting. It looks like it will be a powerful tool for doing exploratory work with large time series and comparing and contrasting multiple time series. Moreover, I expect that time series experts who can fine tune prophet and more traditional time series models will assess its behavior in relation to these models and develop a better intuition as to how to deploy TimeGPT.

For a more sophisticated analysis of these series see the [Electricity Load Forecast Tutorial](https://nixtlaverse.nixtla.io/statsforecast/docs/tutorials/electricityloadforecasting.html).
